  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 xl:grid-cols-4">
        <article className="rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Em Licitacao</p><p className="mt-2 text-3xl font-black text-slate-950">{total}</p><p className="mt-2 text-sm text-slate-600">Processos atualmente dentro da fase licitatoria.</p></article>
        <article className="rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Publicados</p><p className="mt-2 text-3xl font-black text-slate-950">{publicados}</p><p className="mt-2 text-sm text-slate-600">Com condutor e numero de edital ja definidos.</p></article>
        <article className="rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Aguardando publicidade</p><p className="mt-2 text-3xl font-black text-slate-950">{aguardandoPublicacao}</p><p className="mt-2 text-sm text-slate-600">Processos em Licitacao que ainda nao foram publicados.</p></article>
        <article className="rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Operacao do modulo</p><p className="mt-2 text-3xl font-black text-slate-950">Fase</p><p className="mt-2 text-sm text-slate-600">Publicidade, edital e controle especifico da licitacao.</p></article>
      </div>

      <SectionCard
        title="Modulo de Licitacao"
        description="Esta tela nao substitui o cadastro geral de Processos. Aqui ficam apenas as etapas especificas da fase licitatoria."
        action={
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.95fr]">
          <div className="space-y-4">
              ...
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50 text-left text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
                  <tr>
                    ...
                    <th className="px-4 py-3">Ultima movimentacao</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={row.processoId} onClick={() => setSelectedProcessId(row.processoId)} className={["cursor-pointer transition", active ? "bg-sky-50/80" : "hover:bg-slate-50"].join(" ")}>
                        <td className="px-4 py-3 align-top">
                          ...
                          <div className="text-xs text-slate-500">{row.modalidade ?? "Modalidade nao definida"}</div>
                        </td>
                        ...
                        <td className="px-4 py-3 align-top">
                          ...
                          <div className="mt-2 text-xs text-slate-500">{row.numeroEdital ?? "Edital ainda nao gerado"}</div>
                        </td>
                  ...
                  {!rows.length ? (
                    <tr>
                      <td className="px-4 py-8 text-center text-slate-500" colSpan={5}>
                        {listQuery.isFetching ? "Carregando processos da Licitacao..." : "Nenhum processo esta na fase de Licitacao. Use o Workflow para mover um processo para este modulo."}
                      </td>
                    </tr>
                  ) : null}
                </tbody>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm text-slate-600">Exibindo <span className="font-bold text-slate-950">{rows.length}</span> de <span className="font-bold text-slate-950">{total}</span> processos em Licitacao.</p>
          ...
          <div className="space-y-4">
            <SectionCard
              ...
              description="Controle da publicidade e do edital apenas para o processo selecionado em Licitacao."
              action={<div className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-white"><ScrollText className="h-4 w-4" />Operacao especifica</div>}
            >
              {!selectedProcessId ? (
                <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-sm text-slate-500">Selecione um processo da fase de Licitacao para operar.</div>
              ) : detailQuery.isLoading ? (
                <div className="space-y-3">{[0, 1, 2, 3].map((item) => <div key={item} className="h-16 animate-pulse rounded-3xl bg-slate-100" />)}</div>
              ) : detailQuery.error ? (
                <div className="rounded-3xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-900">Falha ao carregar o detalhe da Licitacao.</div>
              ) : (
                <div className="space-y-4">
                  <article className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Processo em Licitacao</p>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <article className="rounded-3xl border border-slate-200 bg-white px-4 py-4"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Etapa atual</p><p className="mt-2 text-lg font-black text-slate-950">{detailQuery.data?.estado?.etapaAtual ?? "-"}</p><p className="mt-1 text-sm text-slate-600">{detailQuery.data?.processo?.modalidade ?? "Modalidade nao definida"}</p></article>
                    <article className="rounded-3xl border border-slate-200 bg-white px-4 py-4"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Publicidade</p><p className="mt-2 text-lg font-black text-slate-950">{detailQuery.data?.processo?.numeroEdital ?? "Nao publicado"}</p><p className="mt-1 text-sm text-slate-600">{detailQuery.data?.processo?.condutorProcesso?.nome ?? "Condutor ainda nao definido"}</p></article>
                  </div>
                  ...
                  {detailQuery.data?.processo?.publicado ? (
                    <article className="rounded-3xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-900">
                      ...
                      <p className="mt-1 leading-6">As demais etapas especificas da Licitacao podem continuar a partir desta base.</p>
                    </article>
                  ) : (
                    <article className="rounded-3xl border border-slate-200 bg-white px-4 py-4">
                      ...
                      <form className="space-y-4" onSubmit={handlePublishProcesso}>
                        <div className="rounded-3xl border border-sky-100 bg-sky-50 px-4 py-4 text-sm text-sky-900">
                          <p className="font-semibold">Regra da publicacao</p>
                          <p className="mt-1 leading-6 text-sky-800">Ao publicar, o sistema exige o condutor e gera automaticamente o numero do edital no padrao da modalidade, por exemplo `PE-001-2026`.</p>
                        </div>
