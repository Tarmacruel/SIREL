...
export function PlanejamentoDfdPage({ processoId }: PlanejamentoDfdPageProps) {
  ...
  const solicitanteNome = detalhe?.dfd?.solicitante?.name ?? authQuery.data?.user.name ?? "Será registrado ao salvar";
  const adminSecretaria = findSecretariaAdministracao(catalogQuery.data?.secretarias ?? []);
  const secretariaResponsavel =
    detalhe?.dfd?.secretariaResponsavel?.nome ??
    (form.demandaSistemica ? adminSecretaria?.nome ?? "Secretaria de Administração" : detalhe?.processo.secretaria ?? "-");
                ...
                <form className="space-y-4" onSubmit={handleSave}>
                  <div className="grid gap-3 md:grid-cols-2">
                    <FormField label="Solicitante">
                      <Input value={solicitanteNome} disabled />
                    </FormField>
                    <FormField label="Secretaria responsável">
                      <Input value={secretariaResponsavel} disabled />
                    </FormField>
                  </div>

                  <div className="grid gap-3 md:grid-cols-2">
                    <FormField label="Setor demandante" error={fieldErrors.setorDemandante}>
                      <Input
                        value={form.setorDemandante}
                        error={Boolean(fieldErrors.setorDemandante)}
                        onChange={(event) => setForm((current) => ({ ...current, setorDemandante: event.target.value }))}
                      />
                    </FormField>
