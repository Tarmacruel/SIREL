...
export const dfd = pgTable("dfd", {
  ...
  secretariaDemandanteId: integer("secretaria_demandante_id").references(() => secretarias.id),
  ...
  solicitantePessoaId: integer("solicitante_pessoa_id").references(() => pessoas.id),
  solicitanteUserId: integer("solicitante_user_id").references(() => users.id),
  assinaturaResponsavelId: integer("assinatura_responsavel_id").references(() => pessoas.id),
...
export const itensProcesso = pgTable("itens_processo", {
  ...
  catalogoItemId: integer("catalogo_item_id").references(() => catalogoItens.id, { onDelete: "set null" }),
  ...
  idxCatalogoItem: index("itens_processo_catalogo_item_idx").on(table.catalogoItemId)
}));
...
export const contratoItens = pgTable("contrato_itens", {
  id: serial("id").primaryKey(),
  contratoId: integer("contrato_id").notNull().references(() => contratos.id, { onDelete: "cascade" }),
  catalogoItemId: integer("catalogo_item_id").notNull().references(() => catalogoItens.id, { onDelete: "cascade" }),
  descricao: text("descricao").notNull(),
  unidade: varchar("unidade", { length: 32 }).notNull(),
  quantidadeContratada: numeric("quantidade_contratada", { precision: 14, scale: 3 }).notNull(),
  quantidadeConsumida: numeric("quantidade_consumida", { precision: 14, scale: 3 }).notNull().default("0"),
  valorUnitario: numeric("valor_unitario", { precision: 14, scale: 2 }),
  ativo: boolean("ativo").notNull().default(true),
  criadoEm: timestamp("criado_em", { withTimezone: true }).notNull().defaultNow(),
  atualizadoEm: timestamp("atualizado_em", { withTimezone: true }).notNull().defaultNow()
  ...
  idxCatalogoItem: index("contrato_itens_catalogo_item_idx").on(table.catalogoItemId),
  uqContratoItem: uniqueIndex("contrato_itens_contrato_catalogo_uq").on(table.contratoId, table.catalogoItemId)
}));
